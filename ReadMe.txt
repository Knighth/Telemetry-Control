...
Telemetry Control v1.2.1-f1 Build 01
------------------------------------
...
So why use this mod?
If you are blocking via the url or ip method it's all or nothing, maybe you don't mind sending certain information. Also doing so produces errors in your logs, requests still get sent and processes have to wait for a responses that will not come, why not just stop said requests from happening in the first place. Also maintaining the other methods could be pain if depending on the particulars of the situation. Maybe you want to see what's trying to be sent, if so there is a setting for that in the mod.
...
That said, while I actually think the default for sending this info should probably be set to 'on' they've provided no 'off' setting, which very much annoys me. Additionally while in some edge cases like matching certain info with a specific open pdx support case, it also seems excessive to associate my steamid and pdx account information with every thing I do.

What's with this disable workshop ad option?
Honestly it's why I first started coding this, I was getting annoyed that 'feature' could not be turned off. Again it's cool and useful feature for some I guess, but would an off-switch have killed them, I mean it's a tiny amount of wasted bandwith but it's still a waste if I don't need it, and I constantly leave my game at the main menu for hours. So in the course of figuring out how to disable it I figured why not disable telemetry while I'm at it, though personally I was already using a patched dll for that.
